...
function validateCurrentStep() {
    ...
    switch (currentStep) {
        ...
        case 'data':
            ...
            if (!hasRevenueData && !hasChannelData) {
                showNotification('Please upload revenue data', 'error');
                return false;
            }
            break;
    }
...
function initializeChannelTabs() {
    ...
    if (!channelUploadContent) return;
    
    // Initialize channel tabs if needed
...
// Duplicate function removed - using the one at line 1476
                ...
                <div class="channel-breakdown">
                    ...
                    ${Object.entries(results.channelPerformance || {}).map(([channel, data]) => `
                        <div class="channel-result">
                            ...
                            <span class="channel-cac">$${data.cac}</span>
                            <span class="channel-customers">${data.customers} customers</span>
                            <span class="channel-spend">$${data.spend.toFixed(2)} spend</span>
                        </div>
                    `).join('')}
                </div>
            </div>
